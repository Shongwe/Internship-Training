...
def test_update_item(client):
    create = client.post("/api/items", json={"name": "Old", "description": "Temp", "price": 5})
...
def test_blank_name_validation(client):
    response = client.post("/api/items", json={"name": "   ", "description": "Bad", "price": 10})
    assert response.status_code == 422

def test_negative_price_validation(client):
    response = client.post("/api/items", json={"name": "BadPrice", "description": "Oops", "price": -1})
    assert response.status_code == 422

def test_missing_price_validation(client):
    response = client.post("/api/items", json={"name": "NoPrice", "description": "Oops"})
    assert response.status_code == 422
...
def test_multiple_items_id_increment(client):
    r1 = client.post("/api/items", json={"name": "First", "description": "One", "price": 1})
    r2 = client.post("/api/items", json={"name": "Second", "description": "Two", "price": 2})
